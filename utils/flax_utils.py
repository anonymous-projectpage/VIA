...
def save_actor(agent, save_dir, epoch):
    """Save only the actor (all 'actor' param groups) to actor_<epoch>.pkl.

    For tbd/via-style agents only modules_actor_bc_flow (+ encoder) is saved,
    which is much lighter than saving the whole agent.
    """
    actor_params = {k: v for k, v in agent.network.params.items() if 'actor' in k}
    save_dict = flax.serialization.to_state_dict(actor_params)
    save_path = os.path.join(save_dir, f'actor_{epoch}.pkl')
    with open(save_path, 'wb') as f:
        pickle.dump(save_dict, f)
    print(f'Saved actor to {save_path}')
